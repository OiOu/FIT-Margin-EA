...
public class HStoreType implements UserType {
     ...
     */
    private static final Pattern HSTORE_ENTRY_PATTERN =
        Pattern.compile(String.format("\"(.*)\"%s\"(.*)\"", HSTORE_SEPARATOR_TOKEN));
    ...
    private Map<String, Object> convertToEntityAttribute(final String dbData) {
        ...
        if (dbData != null) {
            ...
            while (tokenizer.hasMoreTokens()) {
                final Matcher matcher = HSTORE_ENTRY_PATTERN.matcher(tokenizer
                    .nextToken().trim());
                if (matcher.find()) {
